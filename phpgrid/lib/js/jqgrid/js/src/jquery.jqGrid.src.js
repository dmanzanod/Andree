...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		multiSort = function(iCol, obj ) {
			...
			// azg: fixed multiSort function to maintain fields order
			// get previous sorting 
			var _lastsort;
			_lastsort = ts.p.sortname + ts.p.sortorder;

			// console.log('.....new.....');
			// console.log(sort);
			// console.log("last:"+_lastsort);

			var _sort = sort.split(", ");
			var _lastsort = _lastsort.split(", ");
			
			// skip 1, prepend all previous sorting in new sort
			_lastsort = _lastsort.reverse();
			for (var l in _lastsort)
				if (_lastsort[l].substr(0,1) != "1")
					_sort.unshift(_lastsort[l]);

			// console.log("build:");
			// console.log(_sort);

			// remove all duplicates sort (result of prepend)
			var _sortdup = [];
			for(var s in _sort)
				_sortdup[_sort[s]] = 1;

			_sort = [];
			for(var s in _sortdup)
				_sort.push(s);

			// console.log("removing dups:");
			// console.log(_sort);

			// give priority to new sort order if same field exist
			var _sort_rev = _sort.reverse();
			// console.log("rev:"+_sort_rev);
			var _tmp_rev = [];
			for(var s in _sort_rev)
			{
				var t = _sort_rev[s].split(" ");
				t = t[0];

				if(_tmp_rev[t])
				{
					_sort_rev[s] = _sort_rev[_tmp_rev[t]];
					delete(_sort_rev[_tmp_rev[t]]);
				}
				else
					_tmp_rev[t] = s;
			}
			// remove deleted
			var _sort_rev = _sort_rev.filter(function (el) { return el; });
			_sort_rev = _sort_rev.reverse();
			
			// console.log("removing dup order:");
			// console.log(_sort_rev);

			// remove unsorted fields, not in new sort and prepended by previous sort
			var _newsort = sort.split(", ");
			for(var s in _sort_rev)
			{
				var t = _sort_rev[s].split(" ");
				t = t[0];
				...
				var found=0;
				for (var n in _newsort)
				{
					var t_new = _newsort[n].split(" ");
					t_new = t_new[0];
			...
			// remove deleted
			var _sort = _sort_rev.filter(function (el) { return el; });

			// console.log("final:");
			// console.log(_sort);
			
			// remove hidden column from sorting as non controllable
			for(var s in _sort)
			{
				var t = _sort[s].split(" ");
				t = t[0];
				$.each(cm, function(i){
					if (cm[i].name == t && cm[i].hidden == true)
					{
						delete(_sort[s]);;
						return;
					}	
			...
			var _sort = _sort.filter(function (el) { return el; });

			// set for grid
			sort = _sort.join(", ");
			// console.log(sort);
			ls = sort.lastIndexOf(" ");
			ts.p.sortname = sort.substring(0, ls+1);
			ts.p.sortorder = sort.substring(ls+1);

			// console.log(ts.p.sortname);
			// console.log(ts.p.sortorder);
		},
		...
		for(i=0;i<this.p.colNames.length;i++){
			var tooltip = ts.p.headertitles ? (" title=\""+ $.jgrid.stripHtml( ts.p.colModel[i].tooltip || ts.p.colNames[i] )+"\"") :"";
		...
		$(ts).before(grid.hDiv).click(function(e) {
			...
			if(ts.p.cellEdit === true) {
				if(ts.p.multiselect && scb && cSel){
					...
					// azg: single click editing on excel
					var cmode = false;
					if (ts.p.cellEditMode == "click") cmode = true;
					
					try {$(ts).jqGrid("editCell",ri,ci,cmode);} catch (_) {}
				}
				return;
			}
			if (!cSel) {
		...
		.dblclick(function(e) {
			...
			if(ts.p.cellEdit === true && ts.p.cellEditMode != "click") {
				var scb = $(td).hasClass("cbox"),
				cSel = $(ts).triggerHandler("jqGridBeforeSelectRow", [ptr[0].id, e]);
				cSel = (cSel === false || cSel === 'stop') ? false : true;
				if ($.isFunction(ts.p.beforeSelectRow)) {
		...
		return this.each(function (){
			...
			if (cm.editable===true && ed===true && !cc.hasClass("not-editable-cell")) {
				...
				$("input, select, textarea",cc).bind("keydown",function(e) {
					...
					if (e.keyCode === 13 && !e.shiftKey && !jQuery('.ui-autocomplete').is(':visible')) {
					...
					{
						if (navigate && !jQuery('.ui-autocomplete').is(':visible'))
						{
							$($t).jqGrid("saveCell",iRow,iCol);
				...
				$.each($t.p.colModel,function(){
					...
					switch (this.stype) {
						case 'select' :
							...
							// azg: if multiselect filter linked, reset it on reload
							if ($elem.next('.ui-multiselect').length) $elem.multiselect('uncheckAll');
                            var multiselect_filter = ($elem.next('.ui-multiselect').length == 1);

                            // azg: reset select2 in autofilter
                            if($elem.next('.select2').length)
                                $elem.val('').select2();
						
							$elem.find("option").each(function (i){
								// azg: skip first selected for multiselect-filter case
								if(i===0 && !multiselect_filter) { this.selected = true; }
						...
						case 'text':
							...
							// azg: if daterange filter linked, reset it on reload
							if ($elem.next('.comiseo-daterangepicker-triggerbutton').length) $elem.daterangepicker("clearRange");
	...
	editGridRow : function(rowid, p){
		...
		return this.each(function(){
			...
			if(rp_ge[$t.p.id].checkOnSubmit || rp_ge[$t.p.id].checkOnUpdate) {
				...
				// azg: removed cancel button
				bC  =""; //<a id='cNew' class='fm-button ui-state-default ui-corner-all' style='z-index:1002'>"+p.bExit+"</a>";
				var zI = p.zIndex  || 999;zI ++;
				$("<div class='"+ p.overlayClass+" jqgrid-overlay confirm' style='z-index:"+zI+";display:none;'>&#160;"+"</div><div class='confirm ui-widget-content ui-jqconfirm' style='right:25%;left:25%;padding-top:20px;z-index:"+(zI+1)+"'>"+p.saveData+"<br/><br/>"+bS+bN+bC+"</div>").insertAfter("#"+frmgr);
			...
			$("#sData", frmtb+"_2").click(function(){
				...
				if(postdata[$t.p.id+"_id"] === "_empty")	{postIt();}
					...
					// diff = compareData(postdata,rp_ge[$t.p.id]._savedData);
					// azg: show confirm each time, commented above
					diff = true;
					if(diff) {
						$("#"+frmgr).data("disabled",true);
				...
				if(grp.showSummaryOnHide) {
					if(r){
						while(r) {
							...
							// azg: fix copied from last commit github, was hiding summary even showSummaryOnHide:true
				...
				if ( nm !== 'cb' && nm !== 'subgrid' && cm.editable===true && nm !== 'rn' && !$(this).hasClass('not-editable-cell')) {
					switch (cm.edittype) {
                        ...
                        case "button" :
                            ...
                            break;
						case 'textarea':
                            tmp[nm]=$("textarea.editable, textarea.FormElement",this).val();
							break;
						case 'select':
							if(!cm.editoptions.multiple) {
					...
					cmItem = $.extend({}, agr1, {
						...
						label: $.isFunction(label) ?
									...
									(jgrid.template.apply(self, colType === 2 ?
											[String(label), agr1.label, agr1.aggregator, agr1.member, iAggr] :
											[String(label), agr1.label, agr1.aggregator, agr1.member, yIndex.getItem(iyData)[level], level]))
					});
